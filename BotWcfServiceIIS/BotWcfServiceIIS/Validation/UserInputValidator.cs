...
namespace BotWcfServiceIIS.Validation
{
    /// <summary>
    ...
    public static class UserInputValidator
    {
        ...
        static UserInputValidator()
        {
            ...
            UserNameRule = string.Format(
                ...
                UsernameMinimumSymbols );

            PasswordMinimumSymbols = 6;
            PasswordMaximumSymbols = int.MaxValue; //no upper bound, yet
            PasswordRule = string.Format(
                "Passwords must contain only letters, digits, underscore and a dot symbols."
                + "Also it must contain a letter and a password. "
        ...
        {
            if (!InRange( UsernameMinimumSymbols, username.Length, UsernameMaximumSymbols )
        ...
        {
            if (!InRange( PasswordMinimumSymbols, password.Length, PasswordMaximumSymbols )
